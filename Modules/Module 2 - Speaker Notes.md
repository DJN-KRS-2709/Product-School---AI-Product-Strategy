...
Same move as Module 1 — tell one company's story across all three takeaways. In M1 you used Gamma. For M2, the company that maps cleanly onto all three lessons is **Cursor**. Pick one beat for each takeaway and let the narrative carry the lesson. The slide stays clean — three bullets — but the delivery is a continuous story.

Let me tell you a story that ties together everything we did today. Cursor — the AI code editor. When they launched in 2023, the product was, on paper, indefensible. A wrapper around GPT-4. VS Code with an AI panel. Anyone could have built it in a weekend, and a lot of people pointed that out. The founders didn't argue. They publicly said — multiple times — "we don't have a model advantage." Most founders would never admit that out loud. But that honesty is what let them build the right thing. Instead of pretending they had a moat at the model layer, they built one somewhere else: every accept, every reject, every diff between what their AI suggested and what the developer actually shipped became proprietary signal. Every keystroke. Compounding. Not stored in a database — running through a loop. That's takeaway one: **data in a database isn't a moat — loops are.** A stockpile doesn't compound. A product that gets smarter every time someone uses it does.

Now, here's where it gets interesting. The teams getting eaten in the same window — Jasper, the early ChatGPT wrappers — were giving themselves 4s and 5s on moats they didn't actually have. Their pitch decks said "proprietary AI" and "data advantage" and all the things investors wanted to hear. Cursor scored themselves a 1 or 2 on the model-moat axis and used that honesty to make sharper decisions. They invested in editor depth, in workflow embedding, in the things they could actually defend — not the things they wished they could defend. If they'd graded their own homework generously, they'd have built the wrong product. That's takeaway two: **self-assessment is always generous.** A partner attacking your moat — like the 90-day encroachment plan you just survived — surfaces blind spots that pretending-everything-is-fine cannot. The teams that get blindsided are almost always the ones who never let anyone push on their scores.
